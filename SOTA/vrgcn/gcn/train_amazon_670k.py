...
import l2o_lwgcn_utils as utils
...
flags.DEFINE_integer('epochs', 100, 'Min number of epochs to train.')
flags.DEFINE_integer('data', 0, 'Max amount of visited data')
flags.DEFINE_integer('hidden1', 128, 'Number of units in hidden layer 1.')
flags.DEFINE_float('dropout', 0, 'Dropout rate (1 - keep probability).') # 0.5
...
flags.DEFINE_integer('degree', 2, 'Neighbour subsampling size')
flags.DEFINE_integer('batch_size', 1024, 'Minibatch size for SGD')
flags.DEFINE_bool('cv', False, "Control variate")
flags.DEFINE_bool('preprocess', True,  'Preprocess first aggregation')

flags.DEFINE_integer('test_batch_size', 256, 'Testing batch size')
...
flags.DEFINE_integer('num_fc_layers', 2, 'Number of FC layers')
...
flags.DEFINE_bool('layer_norm', True, 'Layer normalization')
...
# Load data
num_data, train_adj, full_adj, features, train_features, test_features, labels, train_d, val_d, test_d = utils.amazon_670k_loader()


# print(test_d)
# assert False
# train_d = np.array([i for i in range(1208)], dtype=np.int32)
# val_d = np.array([i for i in range(1208, 1708)], dtype=np.int32)
...
def SGDTrain():
    ...
    print(total_time, total_time / FLAGS.epochs, FLAGS.epochs)
